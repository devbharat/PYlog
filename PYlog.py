...
class sdlog2_pp:    
    ...
    def parseMsgDescr(self):
        ...
        if msg_type != self.MSG_TYPE_FORMAT:
            ...
            self.msg_structs[msg_name] = struct.Struct(msg_struct)
    ...
    def parseMsg(self, msg_descr):
        ...
        if (show_fields != None):
            if runningPython3:
                data = list(self.msg_structs[msg_name].unpack(self.buffer[self.ptr+self.MSG_HEADER_LEN:self.ptr+msg_length]))
            else:
                data = list(self.msg_structs[msg_name].unpack(str(self.buffer[self.ptr+self.MSG_HEADER_LEN:self.ptr+msg_length])))
